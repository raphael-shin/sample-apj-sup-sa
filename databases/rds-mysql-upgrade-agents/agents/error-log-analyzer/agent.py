"""
MySQL Error Log Analyzer Agent
Reads CloudWatch Logs from Green instance (error/mysql-error-running.log)
Analyzes logs and returns upgrade considerations from MySQL 8.X to 8.4.X
...
AGENT_NAME = "rds_mysql_error_log_analyzer"
...
def fetch_cloudwatch_logs(log_group_name: str, log_stream_name: str = None,
    ...
    """
    Fetch MySQL error logs from CloudWatch Logs
...
def build_language_directive(language: str) -> str:
    ...
    if (language or "ko").strip().lower() == "en":
        return (
            ...
            "- Keep MySQL identifiers, log messages, SQL, and metric names "
            "unchanged.\n"
        )
    return (
        "**출력 언어: 한국어**\n"
...
def _strip_report_time(markdown: str) -> str:
    ...
    text is touched, so the log time range ("분석 기간") and data are never altered.
    """
...
async def invoke(payload):
    ...
    {
        "log_group_name": "/aws/rds/instance/mysql-green/error",
    ...
    if not log_group_name:
        return {
            ...
            "message": "log_group_name is required (e.g., /aws/rds/instance/mysql-green/error)"
        }
    ...
    agent_message = f"""
{language_directive}
MySQL Error Log 분석 및 업그레이드 주의사항 추출 후 S3에 마크다운 보고서 저장.

**작업:**
1. fetch_cloudwatch_logs로 CloudWatch Logs에서 MySQL error log 수집
...
3. 로그를 분석하여 MySQL 8.X에서 8.4.X로 업그레이드 시 주의사항 추출 (마크다운 형식)
...
- MySQL 8.4.X에서 변경된 사항과 관련된 내용 강조
...
- 발견된 문제가 없어도 일반적인 MySQL 8.4.X 업그레이드 주의사항은 제공할 것

**리포트 구조:**
```markdown
# MySQL Error Log 분석 결과
