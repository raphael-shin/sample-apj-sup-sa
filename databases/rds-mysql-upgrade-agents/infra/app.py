#!/usr/bin/env python3
"""CDK app entry point for MySQL Upgrade multi-agent deployment."""
import os
import re
import secrets
import sys
...
from cdk_rds_mysql_upgrade.stack import RdsMysqlUpgradeAgentStack
...
RdsMysqlUpgradeAgentStack(
    ...
    agent_names={
        ...
        "variables_compare": os.environ.get(
            "VARIABLES_COMPARE_NAME", f"rds_mysql_variables_compare_{suffix}"
        ),
        "error_log_analyzer": os.environ.get(
            "ERROR_LOG_ANALYZER_NAME", f"rds_mysql_error_log_analyzer_{suffix}"
        ),
        "upgrade_readiness": os.environ.get(
            "UPGRADE_READINESS_NAME", f"rds_mysql_upgrade_readiness_analyzer_{suffix}"
        ),
    },
)
