...
# Aurora MySQL Upgrade Readiness Checker — Streamlit UI (optional)
...
- An **active Aurora MySQL Blue-Green Deployment** (Blue 3.04 / Green 3.10) that
  the agents can reach — see the "Aurora MySQL environment" section in the
  [root README](../../README.md#prerequisites) for the full run-time conditions
- Python 3.10+
...
cp .env.example .env
# Enter only the CDK Outputs (6 ARNs) in .env
...
2. `./.env` — values specific to this UI (mainly the 6 ARNs produced by CDK)
...
Just copy these 6 into `./.env` and you're done.
...
- **Sidebar reports** — download the 5 `.md` reports after analysis completes
