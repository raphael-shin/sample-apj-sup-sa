...
Aurora MySQL 마이너 업그레이드 (3.04 → 3.10) 준비 상태를 자동 분석하는 AWS
Bedrock AgentCore 기반 multi-agent 패키지. 고객 AWS 계정에 CDK 로 바로 배포할
수 있는 자체 완결 구조입니다.
...
| **MySQL 호환 버전** | 양쪽 모두 MySQL 8.0 호환 (마이너 업그레이드) |
| **AWS 서비스** | Amazon Aurora MySQL-compatible |
| **리전** | `us-east-1` 중심 검증. 다른 리전은 Bedrock AgentCore 가용 여부 확인 필요 |
| **CDK** | AWS CDK v2 (Python) |
| **Runtime** | Bedrock AgentCore (Linux ARM64 컨테이너) |

> 위 범위 밖의 조합 (예: Aurora MySQL 2.x, RDS for MySQL, 메이저 업그레이드) 은
> 미검증이며 동작을 보장하지 않습니다. 메이저 업그레이드용은 별도 프로젝트
> [`mysql-upgrade-agents`](https://github.com/nasaoreo/mysql-upgrade-agents) 참조.
...
│   ├── upgrade-readiness/        #   InnoDB status + 옵티마이저 패턴 리스크 분석
│   ├── query-risk-scorer/        #   performance_schema digest 기반 위험도 점수
│   └── plan-diff/                #   Blue/Green EXPLAIN FORMAT=JSON 실측 비교
│       ├── Dockerfile
│       ├── agent.py
│       └── requirements.txt
│
...
- **IAM Role** — 6개 agent 가 공유하는 실행 역할
...
- **AgentCore Runtime × 6** — 고객 VPC 내부에서 실행
...
### Aurora MySQL 환경 (실행 시점에 필요)

이 패키지는 **활성 상태의 Aurora MySQL Blue-Green 배포**를 분석합니다 — 양쪽
클러스터에 네트워크로 접속해서 파라미터, 상태, 통계를 읽어옵니다. 다음 조건은
**orchestrator 를 실행하기 전에** 갖춰져 있어야 합니다 (배포 시점이 아니라 실행
시점 기준):

- **활성 Blue-Green 배포가 존재해야 함.** 첫 단계(`check_blue_green_deployment`)가
  Blue 와 Green **양쪽** 클러스터에 접속하며, 한쪽이라도 실패하면 워크플로가 즉시
  중단됩니다. Green 이 없는 단일 클러스터로는 동작하지 않습니다.
- **Blue = Aurora MySQL 3.04.x, Green = Aurora MySQL 3.10.x.** 분석이 이
  마이너 업그레이드 조합(양쪽 모두 MySQL 8.0 호환)을 전제로 설계되어 있습니다.
- **양쪽 cluster endpoint 가 3306 포트로 접근 가능해야 함** — AgentCore 에 지정한
...
- **Green 클러스터의 error log 가 CloudWatch Logs 로 export 되어 있어야 함**
  (아래 `GREEN_LOG_GROUP` 참고). 없으면 Error Log Analyzer 가 읽을 대상이 없습니다.

> 왜 Blue-Green 인가? 활성 Blue-Green 배포가 있으면 현재(3.04)와 업그레이드된
> (3.10) 클러스터가 나란히 존재하므로, 에이전트가 각각에서 파라미터와 DB 상태를
> 읽어 switchover 전에 직접 비교할 수 있습니다.
...
| `REPORTS_BUCKET_NAME` | 리포트용 S3 버킷 이름 prefix. 실제 버킷명은 `<prefix>-<DEPLOYMENT_SUFFIX>` 로 자동 생성됨 (예: `aurora-mysql-upgrade-reports-d2be5a`). 소문자/숫자/`-` 만 허용 |
...
| `BLUE_HOST` / `GREEN_HOST` | Blue(Aurora MySQL 3.04) / Green(Aurora MySQL 3.10) **cluster endpoint** (인스턴스 endpoint 아님) |
...
않아도 됩니다. `ui/.env` 에는 `cdk deploy` 결과로 나온 **6개 Agent ARN** 만
붙여넣으면 됩니다.

```bash
cd ui/streamlit
pip install -r requirements.txt
cp .env.example .env        # ARN 6개만 입력
...
| `Cannot connect to the Docker daemon` (Finch 사용 시) | `finch vm start` 후 `export CDK_DOCKER=finch` (또는 `infra/.env` 에 `CDK_DOCKER=finch` 추가) |
