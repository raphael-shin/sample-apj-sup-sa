"""
Aurora MySQL Upgrade Readiness Analyzer Agent
Analyzes Blue cluster for Aurora MySQL 3.04 → 3.10 minor upgrade readiness:
...
AGENT_NAME = "aurora_upgrade_readiness_analyzer"
...
def collect_query_risk_data(host: str, port: int = 3306) -> str:
    ...
    Identifies queries that may experience cost-estimation or plan shifts after a
    minor Aurora MySQL upgrade (3.04 → 3.10).
...
def _strip_report_time(markdown: str) -> str:
    ...
    hallucinate the year). We delete it deterministically:
      - a standalone bold line like `**분석 일시:** ...`  → drop the whole line
      - an inline segment like `... | 분석 일시: X | ...`  → drop just that segment
...
async def invoke(payload):
    ...
    agent_message = f"""
{language_directive}
Aurora MySQL 3.10 업그레이드 준비 분석 - InnoDB 상태 + 쿼리 옵티마이저 리스크 분석

**컨텍스트:** Aurora MySQL 3.04 → 3.10 마이너 업그레이드 (양쪽 모두 MySQL 8.0 호환).
메이저 옵티마이저 교체는 없지만, 마이너 패치에 묶인 비용 추정/실행계획의 미세한
변동, InnoDB 동작 보정, Aurora 옵션 default 변경이 일부 쿼리의 plan 을 흔들 수
있어 사전 점검이 필요합니다.
...
**리포트 형식:**
```markdown
# Aurora MySQL 3.10 업그레이드 준비 분석 리포트

**분석 대상:** Aurora MySQL X.XX.X (Blue)
...
### 마이너 업그레이드 시 plan 변동 가능성이 높은 쿼리

| Risk Score | Risk Types | 실행횟수 | Query (요약) |
|------------|------------|---------|--------------|

### Risk Type 설명
- **HASH_JOIN**: JOIN 비용 추정/실행 경로 변동에 민감
- **DERIVED_MERGE**: derived table 병합 동작에 민감
- **CONDITION_PUSHDOWN**: condition pushdown 적용 여부에 민감
- **RANGE_OPTIMIZER**: range/IN/BETWEEN 최적화 변동에 민감
- **ORDER_BY_OPT**: ORDER BY 인덱스 활용 변동에 민감
- **NULL_FILTER**: NULL 처리 최적화에 민감
- **AHI_IMPACT**: Adaptive Hash Index 적용 효과에 민감

### 권장 조치
- 위 쿼리들에 대해 업그레이드 전후 EXPLAIN/실행시간 비교 (운영 트래픽 또는 동일
  데이터셋 기준)
- regression 발견 시 `optimizer_switch` 또는 옵티마이저 hint 로 plan 고정 검토
```
