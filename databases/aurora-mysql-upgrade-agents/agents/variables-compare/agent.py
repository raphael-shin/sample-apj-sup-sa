"""
Aurora MySQL Blue-Green Variables Comparison Agent
Compares SHOW VARIABLES between Blue (Aurora MySQL 3.04) and Green (Aurora MySQL 3.10)
...
AGENT_NAME = "aurora_variables_compare"
...
def get_variables_summary() -> str:
    ...
    blue_text = f"BLUE CLUSTER (MySQL {blue_version}) - {len(_blue_variables)} variables:\n"
    for var, val in sorted(_blue_variables.items()):
        blue_text += f"{var}={val}\n"

    # Format Green variables
    green_text = f"\nGREEN CLUSTER (MySQL {green_version}) - {len(_green_variables)} variables:\n"
...
def build_language_directive(language: str) -> str:
    ...
    The report templates further down use Korean headings as a structural guide;
...
"""
    ...
    agent_message = f"""
{language_directive}
Blue/Green Aurora MySQL 변수 비교 후 간단한 마크다운 리포트 작성.

**컨텍스트:** Aurora MySQL 3.04(MySQL 8.0 호환) → 3.10(MySQL 8.0 호환) 마이너 업그레이드.
대부분의 변수는 양쪽에 동일하게 존재하며, 차이는 보통 cluster/instance parameter group
변경, `aurora_*` 변수의 신규 추가, 마이너 패치에 묶인 행동 변경에서 발생합니다.
...
- **변경된 변수 표: Blue와 Green 모두 해당 Aurora MySQL 버전의 디폴트 값이면 "(default)" 표시**
- **설명은 간결하게 1~2줄로, 핵심만 작성 (timeout 방지)**

**리포트 구조:**
```markdown
# Aurora MySQL 변수 비교 리포트

**Blue 클러스터:** Aurora MySQL X.XX.X (N개 변수)
**Green 클러스터:** Aurora MySQL Y.YY.Y (M개 변수)
...
Blue 에는 있지만 Green 에서 제거된 변수 목록입니다.
> 마이너 업그레이드(3.04 → 3.10)에서는 보통 비어 있습니다. 항목이 있다면
> 애플리케이션에서 해당 변수를 참조하는지 반드시 확인하세요.
...
Green 에 새로 추가된 변수 목록입니다.
> Aurora 마이너 업그레이드에서는 보통 신규 `aurora_*` 옵션이 한두 개 추가되거나
> 비어 있습니다.
...
양쪽에 모두 존재하지만 값이 다른 변수 목록입니다. 마이너 업그레이드의 실질적
영향은 대부분 이 섹션에 집중됩니다.
- Blue 값이 Aurora MySQL 3.04 디폴트면 "(default)" 표시
- Green 값이 Aurora MySQL 3.10 디폴트면 "(default)" 표시
...
- **변경된 변수**: cluster/instance parameter group 차이로 발생한 값 변경인지,
  Aurora 디폴트 변경인지 구분. 옵티마이저(`optimizer_switch`) / 트랜잭션 격리 /
  `innodb_*` 관련 변수는 테스트 필수.
- **추가된 변수**: 신규 `aurora_*` 변수가 있다면 의미를 확인하고 필요시 명시적으로
  값을 설정 (default 가 의도와 다를 수 있음).
- **삭제된 변수**: 발생하면 마이너 업그레이드 노트에 명시되어 있을 가능성이 높음.
  애플리케이션 코드/모니터링에서 참조 여부 확인.
```
"""
