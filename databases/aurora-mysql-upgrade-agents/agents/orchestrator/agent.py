"""
Aurora MySQL Blue-Green Upgrade Analysis Orchestrator Agent
...
AGENT_NAME = "aurora_upgrade_orchestrator"
...
REPORT_NAMES = {
    ...
    "query_risk": {"ko": "쿼리 리스크 점수 리포트", "en": "Query Risk Score Report"},
    "plan_diff": {"ko": "Plan Diff 리포트", "en": "Plan Diff Report"},
}
...
MESSAGES = {
    ...
    "s1_connect": {"ko": "Blue 클러스터 연결 중... ({host})",
                   "en": "Connecting to the Blue cluster... ({host})"},
    ...
    "s3_prep":    {"ko": "Blue/Green Aurora MySQL 변수 수집 준비 중...",
                   "en": "Preparing to collect Blue/Green Aurora MySQL variables..."},
    ...
    "s4_collect": {"ko": "Blue 클러스터 분석 데이터 수집 중... ({host})",
                   "en": "Collecting analysis data from the Blue cluster... ({host})"},
    ...
    "s5_start":   {"ko": "쿼리 리스크 점수 분석 에이전트 실행 중...",
                   "en": "Running the Query Risk Scorer agent..."},
    "s5_collect": {"ko": "Blue 클러스터 performance_schema digest 수집 중... ({host})",
                   "en": "Collecting performance_schema digests from the Blue cluster... ({host})"},
    "s5_invoke":  {"ko": "쿼리 리스크 점수 분석 에이전트 호출 중... (최대 3분 소요)",
                   "en": "Invoking the Query Risk Scorer agent... (up to 3 min)"},
    "s5_done":    {"ko": "분석 완료: 수집 digest {rows}개 / 리포트 상위 {top}개",
                   "en": "Analysis complete: {rows} digests collected / top {top} in report"},
    "s6_start":   {"ko": "Plan Diff 에이전트 실행 중 (Blue/Green EXPLAIN 비교)...",
                   "en": "Running the Plan Diff agent (Blue/Green EXPLAIN comparison)..."},
    "s6_collect": {"ko": "Blue 클러스터 events_statements_history 수집 중... ({host})",
                   "en": "Collecting events_statements_history from the Blue cluster... ({host})"},
    "s6_invoke":  {"ko": "Plan Diff 에이전트 호출 중... (최대 3분 소요)",
                   "en": "Invoking the Plan Diff agent... (up to 3 min)"},
    "s6_done":    {"ko": "분석 완료: 비교 {total}개 / 동일 {identical} / 변경 {changed} / 실패 {failed}",
                   "en": "Analysis complete: {total} compared / {identical} identical / {changed} changed / {failed} failed"},
    "s7_start":   {"ko": "분석 결과 요약 생성 중...",
                   "en": "Generating the analysis summary..."},
    "s7_init":    {"ko": "LLM 모델 초기화 중... (Claude Sonnet)",
                   "en": "Initializing the LLM model... (Claude Sonnet)"},
    "s7_gen":     {"ko": "요약 텍스트 생성 시작...",
    ...
    "missing_fields": {
        "ko": ("필수 필드가 누락되었습니다: blue_host, green_host, db_secret_id, "
               ...
               "upgrade_readiness_analyzer_arn, query_risk_scorer_arn, plan_diff_arn"),
...
def check_blue_green_deployment(blue_host: str, green_host: str, password: str,
    ...
    Args:
        blue_host: Blue cluster endpoint
        green_host: Green cluster endpoint
    ...
    # TLS + timeout-bounded connection (reused for both clusters).
    ...
    try:
        ...
        logger.info(f"✅ Blue cluster connected: {blue_version}")
    except Exception as e:
        result["blue_status"] = "failed"
        # Log the detail; keep the surfaced message free of host/driver internals.
        result["message"] += f"Blue connection failed: {type(e).__name__}. "
        logger.error(f"❌ Blue cluster connection failed: {type(e).__name__}: {e}")
    finally:
    ...
    try:
        ...
        logger.info(f"✅ Green cluster connected: {green_version}")
    except Exception as e:
        result["green_status"] = "failed"
        result["message"] += f"Green connection failed: {type(e).__name__}. "
        logger.error(f"❌ Green cluster connection failed: {type(e).__name__}: {e}")
    finally:
    ...
    if result["blue_status"] == "connected" and result["green_status"] == "connected":
        ...
        result["message"] = "Blue-Green deployment not ready. Cannot connect to both clusters. " + result["message"]
    else:
        result["status"] = "error"
        if result["blue_status"] == "failed":
            result["message"] = "Cannot connect to Blue cluster. " + result["message"]
        elif result["green_status"] == "failed":
            result["message"] = "Cannot connect to Green cluster. " + result["message"]
...
def run_error_log_analyzer(log_group_name: str, agent_runtime_arn: str,
                           ...
                           language: str = "ko") -> str:
    """
    Run Error Log Analyzer Agent to analyze Aurora MySQL error logs
...
@tool
def run_query_risk_scorer(blue_host: str, db_secret_id: str,
                          agent_runtime_arn: str,
                          username: str = "admin",
                          top_n: int = 30,
                          s3_bucket: str = "aurora-mysql-upgrade-reports",
                          ...
                          language: str = "ko") -> str:
    """
    Run Query Risk Scorer Agent — runs a performance_schema digest query on
    the Blue cluster and produces a weighted risk-score report scoped to
    Aurora MySQL 3.04 → 3.10 minor upgrade plan-shift exposure.

    Args:
        blue_host: Blue cluster endpoint
        db_secret_id: Secrets Manager secret name or ARN
        agent_runtime_arn: Query Risk Scorer agent ARN
        username: Database username
        top_n: Number of top digests to include in the LLM-driven report
        s3_bucket: S3 bucket for report storage
        region: AWS region
    ...
    Returns:
        JSON string with presigned_url and analysis summary
    """
    logger.info(f"🔧 [{INVOCATION_ID}] TOOL run_query_risk_scorer STARTED")
    start_time = time.time()
    ...
    try:
        ...
        session_id = f"orch-query-risk-{int(time.time())}-{INVOCATION_ID}"

        logger.info(f"📤 Invoking Query Risk Scorer agent with 5min timeout...")
        response = agentcore.invoke_agent_runtime(
            agentRuntimeArn=agent_runtime_arn,
            runtimeSessionId=session_id,
            payload=json.dumps(payload).encode()
        ...
        response_body = response['response'].read().decode('utf-8')
        logger.info(f"📥 [{INVOCATION_ID}] Query Risk Scorer raw response: {response_body[:500]}...")
        response_json = json.loads(response_body)
        ...
        result = {
            "status": "success",
            "presigned_url": presigned_url,
            "raw_s3_url": response_json.get("raw_s3_url", ""),
            "rows_collected": response_json.get("rows_collected", 0),
            "top_n": response_json.get("top_n", top_n),
            "blue_host": blue_host,
        }

        elapsed = time.time() - start_time
        logger.info(f"✅ [{INVOCATION_ID}] TOOL run_query_risk_scorer COMPLETED - {elapsed:.2f}s - presigned_url found: {bool(presigned_url)}")

        return json.dumps(result, ensure_ascii=False)

    except Exception as e:
        elapsed = time.time() - start_time
        error_msg = m("subagent_failed", language, agent="Query Risk Scorer", error=str(e))
        logger.error(f"❌ [{INVOCATION_ID}] TOOL run_query_risk_scorer FAILED - {elapsed:.2f}s - {str(e)}")
...
@tool
def run_plan_diff(blue_host: str, green_host: str, db_secret_id: str,
                  agent_runtime_arn: str,
                  username: str = "admin",
                  s3_bucket: str = "aurora-mysql-upgrade-reports",
                  region: str = "us-east-1",
                  language: str = "ko") -> str:
    """
    Run Plan Diff Agent — pulls recent SELECT statements from Blue's
    events_statements_history, runs EXPLAIN FORMAT=JSON on both Blue and
    Green for each one, and reports plan-shape differences.

    Args:
        blue_host: Blue cluster endpoint
        green_host: Green cluster endpoint
        db_secret_id: Secrets Manager secret name or ARN
        agent_runtime_arn: Plan Diff agent ARN
        username: Database username
        s3_bucket: S3 bucket for report storage
        region: AWS region

    Returns:
        JSON string with presigned_url and totals (identical/changed/explain_failed)
    """
    logger.info(f"🔧 [{INVOCATION_ID}] TOOL run_plan_diff STARTED")
    start_time = time.time()
    ...
    try:
        ...
        session_id = f"orch-plan-diff-{int(time.time())}-{INVOCATION_ID}"

        logger.info(f"📤 Invoking Plan Diff agent with 5min timeout...")
        response = agentcore.invoke_agent_runtime(
            agentRuntimeArn=agent_runtime_arn,
            runtimeSessionId=session_id,
            payload=json.dumps(payload).encode()
        ...
        response_body = response['response'].read().decode('utf-8')
        logger.info(f"📥 [{INVOCATION_ID}] Plan Diff raw response: {response_body[:500]}...")
        response_json = json.loads(response_body)
        ...
        totals = response_json.get("totals", {}) or {}
        result = {
            "status": "success",
            "presigned_url": presigned_url,
            "raw_s3_url": response_json.get("raw_s3_url", ""),
            "queries_collected": response_json.get("queries_collected", 0),
            "identical": totals.get("identical", 0),
            "changed": totals.get("changed", 0),
            "explain_failed": totals.get("explain_failed", 0),
            "blue_host": blue_host,
            "green_host": green_host,
        }

        elapsed = time.time() - start_time
        logger.info(f"✅ [{INVOCATION_ID}] TOOL run_plan_diff COMPLETED - {elapsed:.2f}s - presigned_url found: {bool(presigned_url)}")

        return json.dumps(result, ensure_ascii=False)

    except Exception as e:
        elapsed = time.time() - start_time
        error_msg = m("subagent_failed", language, agent="Plan Diff", error=str(e))
        logger.error(f"❌ [{INVOCATION_ID}] TOOL run_plan_diff FAILED - {elapsed:.2f}s - {str(e)}")
...
async def invoke(payload):
    ...
    4. Run Upgrade Readiness Analyzer (waits for step 3)
    5. Run Query Risk Scorer (waits for step 4)
    6. Run Plan Diff (waits for step 5)
    ...
    Expected payload format:
    {
        ...
        "green_log_group": "/aws/rds/cluster/<green-cluster-id>/error",
        "error_log_analyzer_arn": "arn:aws:...",
        "variables_compare_arn": "arn:aws:...",
        "upgrade_readiness_analyzer_arn": "arn:aws:...",
        "query_risk_scorer_arn": "arn:aws:...",
        "plan_diff_arn": "arn:aws:...",
    ...
    query_risk_scorer_arn = payload.get("query_risk_scorer_arn")
    plan_diff_arn = payload.get("plan_diff_arn")
    ...
    if not all([blue_host, green_host, db_secret_id, green_log_group,
                error_log_analyzer_arn, variables_compare_arn,
                upgrade_readiness_analyzer_arn, query_risk_scorer_arn,
                plan_diff_arn]):
        yield {
            "event_type": "error",
            "step": 0,
    ...
    # ============================================================
    # STEP 5: Run Query Risk Scorer (performance_schema digest 가중 점수)
    # ============================================================
    logger.info(f"🔧 [{INVOCATION_ID}] STEP 5: run_query_risk_scorer")
    yield {
        "event_type": "step_start",
        "step": 5,
        "tool": "run_query_risk_scorer",
    ...
    yield {
        "event_type": "progress",
        "step": 5,
        "message": m("s5_collect", language, host=blue_host)
    }

    yield {
        "event_type": "progress",
        "step": 5,
        "message": m("s5_invoke", language)
    }

    query_risk_result_str = run_query_risk_scorer(
        blue_host=blue_host,
        db_secret_id=db_secret_id,
        agent_runtime_arn=query_risk_scorer_arn,
        username=db_user,
        s3_bucket=s3_bucket,
        region=region,
        language=language
    )
    query_risk_result = json.loads(query_risk_result_str)
    step5_elapsed = time.time() - step5_start

    yield {
        "event_type": "progress",
        "step": 5,
        "message": m("s5_done", language,
                     rows=query_risk_result.get("rows_collected", 0),
                     top=query_risk_result.get("top_n", 0))
    }
    ...
    yield {
        ...
        "result": query_risk_result
    }

    # ============================================================
    # STEP 6: Run Plan Diff (Blue/Green EXPLAIN 비교)
    # ============================================================
    logger.info(f"🔧 [{INVOCATION_ID}] STEP 6: run_plan_diff")
    yield {
        "event_type": "step_start",
        "step": 6,
        "tool": "run_plan_diff",
        "message": m("s6_start", language)
    }

    step6_diff_start = time.time()

    yield {
        "event_type": "progress",
        "step": 6,
        "message": m("s6_collect", language, host=blue_host)
    }

    yield {
        "event_type": "progress",
        "step": 6,
        "message": m("s6_invoke", language)
    }

    plan_diff_result_str = run_plan_diff(
        blue_host=blue_host,
        green_host=green_host,
        db_secret_id=db_secret_id,
        agent_runtime_arn=plan_diff_arn,
        username=db_user,
        s3_bucket=s3_bucket,
        region=region,
        language=language
    )
    plan_diff_result = json.loads(plan_diff_result_str)
    step6_diff_elapsed = time.time() - step6_diff_start

    yield {
        "event_type": "progress",
        "step": 6,
        "message": m("s6_done", language,
                     total=plan_diff_result.get("queries_collected", 0),
                     identical=plan_diff_result.get("identical", 0),
                     changed=plan_diff_result.get("changed", 0),
                     failed=plan_diff_result.get("explain_failed", 0))
    }
    ...
    yield {
        ...
        "step": 6,
        "tool": "run_plan_diff",
        "elapsed_seconds": round(step6_diff_elapsed, 2),
        "result": plan_diff_result
    }

    # ============================================================
    # STEP 7: Generate Summary with LLM
    # ============================================================
    logger.info(f"🤖 [{INVOCATION_ID}] STEP 7: Generating summary with LLM")
    yield {
        "event_type": "step_start",
        "step": 7,
        "tool": "summary_generator",
        "message": m("s7_start", language)
    }

    yield {
        "event_type": "progress",
        "step": 7,
        "message": m("s7_init", language)
    }
    ...
    if query_risk_result.get("presigned_url"):
        report_urls.append({
            "name": report_name("query_risk", language),
            "url": query_risk_result["presigned_url"],
            "type": "query_risk"
        })
    if plan_diff_result.get("presigned_url"):
        report_urls.append({
            "name": report_name("plan_diff", language),
            "url": plan_diff_result["presigned_url"],
            "type": "plan_diff"
        })

    # Create summary agent (no tools, just for text generation).
    # Streaming the summary can exceed the boto3 default 60s read_timeout.
    ...
    summary_system_prompt = {
        "ko": "당신은 Aurora MySQL 마이너 업그레이드(3.04 → 3.10) 분석 결과를 요약하는 전문가입니다. 간결하고 명확하게 한글로 요약하세요.",
        "en": "You are an expert summarizing Aurora MySQL minor upgrade (3.04 → 3.10) analysis results. Summarize concisely and clearly in English.",
    }[language]
    summary_agent = Agent(
        model=summary_model,
    ...
    summary_prompt = f"""다음 Aurora MySQL Blue-Green 마이너 업그레이드(3.04 → 3.10) 분석 결과를 요약해주세요:
...
## 5. 쿼리 리스크 점수 분석 결과
- 상태: {query_risk_result.get('status')}
- 수집 digest 수: {query_risk_result.get('rows_collected', 0)}개
- 리포트 상위 N: {query_risk_result.get('top_n', 0)}

## 6. Plan Diff 분석 결과 (Blue/Green EXPLAIN 비교)
- 상태: {plan_diff_result.get('status')}
- 비교한 쿼리: {plan_diff_result.get('queries_collected', 0)}개
- plan 동일: {plan_diff_result.get('identical', 0)}개
- plan 변경: {plan_diff_result.get('changed', 0)}개
- EXPLAIN 실패: {plan_diff_result.get('explain_failed', 0)}개
...
{"간단한 요약과 주요 액션 아이템 3-5개를 제시해주세요." if language == "ko" else "Provide a brief summary and 3-5 key action items. Write the entire response in English."}"""
    ...
    yield {
        ...
        "step": 7,
        "tool": "summary_generator",
        "elapsed_seconds": round(step7_elapsed, 2),
    ...
    final_result = {
        ...
        "query_risk_scoring": {
            "presigned_url": query_risk_result.get("presigned_url", ""),
            "raw_s3_url": query_risk_result.get("raw_s3_url", ""),
            "status": query_risk_result.get("status"),
            "rows_collected": query_risk_result.get("rows_collected", 0),
            "top_n": query_risk_result.get("top_n", 0),
        },
        "plan_diff_analysis": {
            "presigned_url": plan_diff_result.get("presigned_url", ""),
            "raw_s3_url": plan_diff_result.get("raw_s3_url", ""),
            "status": plan_diff_result.get("status"),
            "queries_collected": plan_diff_result.get("queries_collected", 0),
            "identical": plan_diff_result.get("identical", 0),
            "changed": plan_diff_result.get("changed", 0),
            "explain_failed": plan_diff_result.get("explain_failed", 0),
        },
