...
# aurora-mysql-upgrade-agents

A multi-agent package, built on AWS Bedrock AgentCore, that automatically
analyzes the readiness of an Aurora MySQL minor upgrade (3.04 → 3.10). It is a
self-contained project that can be deployed straight into a customer AWS
account with CDK.
...
| --- | --- |
| **Aurora MySQL (Source)** | 3.04.x |
| **Aurora MySQL (Target)** | 3.10.x |
| **MySQL compatibility** | Both are MySQL 8.0 compatible (minor upgrade) |
| **AWS service** | Amazon Aurora MySQL-compatible |
...
> Combinations outside the range above (e.g. Aurora MySQL 2.x, RDS for MySQL,
> major upgrades) are unverified and not guaranteed to work. For major upgrades,
> see the separate project
> [`mysql-upgrade-agents`](https://github.com/nasaoreo/mysql-upgrade-agents).
...
│   ├── upgrade-readiness/        #   InnoDB status + optimizer pattern risk analysis
│   ├── query-risk-scorer/        #   Weighted risk score from performance_schema digests
│   └── plan-diff/                #   Measured Blue/Green EXPLAIN FORMAT=JSON comparison
│       ├── Dockerfile
│       ├── agent.py
│       └── requirements.txt
│
...
- **IAM Role** — shared execution role for all 6 agents
  (`bedrock-agentcore.amazonaws.com` trust, Bedrock / CWLogs / ECR / VPC ENI / S3 / AgentCore invoke)
- **ECR Images × 6** — orchestrator, variables-compare, error-log-analyzer, upgrade-readiness, query-risk-scorer, plan-diff (ARM64)
- **AgentCore Runtime × 6** — run inside the customer VPC
...
### Aurora MySQL environment (required at run time)

This package analyzes a **live Aurora MySQL Blue-Green Deployment** — it
connects to both clusters over the network to read their parameters, status,
and statistics. The following must already be in place **before you run the
orchestrator** (not at deploy time):

- **An active Blue-Green Deployment exists.** The very first step
  (`check_blue_green_deployment`) connects to **both** the Blue and Green
  clusters; if either connection fails, the workflow stops immediately. A
  standalone cluster with no Green is not enough.
- **Blue = Aurora MySQL 3.04.x, Green = Aurora MySQL 3.10.x.** The analysis is
  built around this minor-upgrade pair (both MySQL 8.0 compatible).
- **Both cluster endpoints are reachable on port 3306** from the subnets /
  security group you give to AgentCore (`SUBNET_IDS` / `SECURITY_GROUP_IDS`).
- **The DB user can read diagnostics.** The default `admin` (or your `DB_USER`)
  needs `SHOW VARIABLES`, `SHOW ENGINE INNODB STATUS`, `SELECT` on
  `performance_schema`, and `EXPLAIN` on the target queries.
...
- **The Green cluster's error log is exported to CloudWatch Logs** (see
  `GREEN_LOG_GROUP` below). Without it the Error Log Analyzer has nothing to read.

> Why Blue-Green? An active Blue-Green Deployment gives you both the current
> (3.04) and the upgraded (3.10) clusters side by side, so the agents can read
> parameters and DB state from each and compare them directly before you
> switch over.
...
| `REPORTS_BUCKET_NAME` | S3 bucket name prefix for reports. The actual bucket name is generated as `<prefix>-<DEPLOYMENT_SUFFIX>` (e.g. `aurora-mysql-upgrade-reports-d2be5a`). Only lowercase letters, digits, and `-` are allowed |
...
| `BLUE_HOST` / `GREEN_HOST` | Blue (Aurora MySQL 3.04) / Green (Aurora MySQL 3.10) **cluster endpoint** (not the instance endpoint) |
...
- `VariablesCompareArn` / `ErrorLogAnalyzerArn` / `UpgradeReadinessArn` / `QueryRiskScorerArn` / `PlanDiffArn`
- `ReportsBucketName`
- `RuntimeRoleArn`
...
payload = {
    ...
    "query_risk_scorer_arn":           "<QueryRiskScorerArn>",
    "plan_diff_arn":                   "<PlanDiffArn>",
}
...
If you'd rather run a GUI than call boto3 directly, see
`ui/streamlit/`. It's a simple app launched locally with
`streamlit run app.py` (not deployed by CDK — runs only on your PC).

The UI **automatically shares** `infra/.env`, so you don't need to re-enter
VPC/DB/host values. In `ui/.env` you only paste the **6 Agent ARNs** produced
by `cdk deploy`.

```bash
cd ui/streamlit
pip install -r requirements.txt
cp .env.example .env        # enter only the 6 ARNs
...
| `Cannot connect to the Docker daemon` (when using Finch) | Run `finch vm start`, then `export CDK_DOCKER=finch` (or add `CDK_DOCKER=finch` to `infra/.env`) |
